...
namespace Llama.Helpers.Mesh
{
    ...
    /// </summary>
    public class Comp_TransferAndTruss : GH_Kernel.GH_Component
    {
        #region Constructors

        /// <summary>
        /// Initialises a new instance of the <see cref="Comp_TransferAndTruss"/> class.
        /// </summary>
        public Comp_TransferAndTruss()
          : base("Transfer and Truss", "T & T",
              "Transfer mesh and dual mesh onto the target's initial configuration, and truss the two together.",
        ...
        protected override void RegisterInputParams(GH_InputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("Height Field", "Z", "Height Field for the vertices of the dual pattern.", GH_Kernel.GH_ParamAccess.tree);
        ...
        protected override void SolveInstance(GH_Kernel.IGH_DataAccess DA)
        {
            ...
            GH_Kernel.Data.GH_Structure<GH_Kernel.Types.GH_Number> struct_HeightFields = new GH_Kernel.Data.GH_Structure<GH_Kernel.Types.GH_Number>();
            ...
            if (!DA.GetDataTree(4, out struct_HeightFields)) { return; } ;

            if (!DA.GetData(5, ref targetFlat)) { return; } ;
            if (!DA.GetData(6, ref targetInitial)) { return; } ;
            ...
            GH.DataTree<double> dualHeightField = new GH.DataTree<double> ();
            for (int i = 0; i < struct_HeightFields.PathCount; i++)
            {
                GH_Kernel.Data.GH_Path path = struct_HeightFields.Paths[i];
                List<GH_Kernel.Types.GH_Number> branch = struct_HeightFields[path];
                for (int j = 0; j < branch.Count; j++)
                {
                    dualHeightField.Add(branch[j].Value, path);
                }
            }
            ...
            #region Transfer Primal Vertices

            List<RH_Geo.Point3d> transferedPrimalVertices = new List<RH_Geo.Point3d>(primalVertices.Count);
            for (int i = 0; i < primalVertices.Count; i++)
            {
                RH_Geo.Point3d position = primalVertices[i];
                RH_Geo.MeshPoint point = targetFlat.ClosestMeshPoint(position, 1e-2);
                ...
                transferedPrimalVertices.Add(transfered);
            }

            #endregion

            #region Transfer Dual Vertices 

            GH.DataTree<RH_Geo.Point3d> transferedDualVertices = new GH.DataTree<RH_Geo.Point3d>();
            for (int i = 0; i < dualVertices.Paths.Count; i++)
            {
                ...
                RH_Geo.MeshPoint point = targetFlat.ClosestMeshPoint(position, 0d);
                ...
                transfered += normal * dualHeightField[path, 0];

                transferedDualVertices.Add(transfered, path);
            }
            ...
            RH_Geo.Mesh flatPrimal = new RH_Geo.Mesh();
            for (int i = 0; i < primalVertices.Count; i++)
            {
                flatPrimal.Vertices.Add(primalVertices[i]);
            }
            for (int i = 0; i < i_PrimalFacesVertices.Paths.Count; i++)
            {
                ...
                AddFace(i_PrimalFaceVertices, ref flatPrimal, tolerance);
            }

            // ----- Transfered Primal Mesh ----- //

            RH_Geo.Mesh transferedPrimal = new RH_Geo.Mesh();
            for (int i = 0; i < primalVertices.Count; i++)
            {
                transferedPrimal.Vertices.Add(transferedPrimalVertices[i]);
            }
            for (int i = 0; i < flatPrimal.Faces.Count; i++) { transferedPrimal.Faces.AddFace(flatPrimal.Faces[i]); }
            for (int i = 0; i < flatPrimal.Ngons.Count; i++) { transferedPrimal.Ngons.AddNgon(flatPrimal.Ngons[i]); }
            ...
            RH_Geo.Mesh flatDual = new RH_Geo.Mesh();
            ...
            for (int i = 0; i < dualVertices.Paths.Count; i++)
            {
                ...
                int i_Vertex = flatDual.Vertices.Add(position);
            ...
            for (int i = 0; i < p_DualFacesVertices.Paths.Count; i++)
            {
                ...
                AddFace(i_DualFaceVertices, ref flatDual, tolerance);
            }


            // ----- Transfered Dual Mesh ----- //

            RH_Geo.Mesh transferedDual= new RH_Geo.Mesh();

            for (int i = 0; i < transferedDualVertices.Paths.Count; i++)
            {
                GH_Kernel.Data.GH_Path path = transferedDualVertices.Paths[i];

                RH_Geo.Point3d position = transferedDualVertices[path, 0];

                int i_Vertex = transferedDual.Vertices.Add(position);
                //i_InitialFaceToDualVertex.Add(path, i_Vertex);
            }
            for (int i = 0; i < flatDual.Faces.Count; i++) { transferedDual.Faces.AddFace(flatDual.Faces[i]); }
            for (int i = 0; i < flatDual.Ngons.Count; i++) { transferedDual.Ngons.AddNgon(flatDual.Ngons[i]); }
            ...
            for (int i = 0; i < i_PrimalFacesVertices.Paths.Count; i++)
            {
                ...
                RH_Geo.Point3d dualPoint = transferedDualVertices[path, 0];

                List<int> i_PrimalFaceVertices = i_PrimalFacesVertices.Branch(path);
                for (int j = 0; j < i_PrimalFaceVertices.Count; j++)
                {
                    RH_Geo.Point3d primalPoint = transferedPrimalVertices[i_PrimalFaceVertices[j]];
            ...
            DA.SetData(0, transferedPrimal);
            DA.SetDataTree(1, diagonals);
            DA.SetData(2, transferedDual);
        ...
        public override Guid ComponentGuid => new Guid("{B05F012B-2237-4F32-82DE-EDB2F2E4BCE4}");
