...
using Gh_Euc3D_Param = BRIDGES.McNeel.Grasshopper.Parameters.Geometry.Euclidean3D;
...
namespace Llama.Helpers.Mesh
{
    ...
    /// </summary>
    internal class Comp_MeshAndDualTrim : GH_Kernel.GH_Component
    {
        #region Constructors

        /// <summary>
        /// Initialises a new instance of the <see cref="Comp_MeshAndDualTrim"/> class.
        /// </summary>
        public Comp_MeshAndDualTrim()
          : base("Mesh And Dual Trim", "MD Trim",
              "Trim a mesh and its dual with a set of boundary polylines defining an connected domain.",
        ...
        {
            pManager.AddParameter(new Gh_Euc3D_Param.Param_HeMesh(), "Trimmed Halfedge Mesh", "T", "Halfedge mesh trimmed with the specified boundary polylines.", GH_Kernel.GH_ParamAccess.item); ;
            pManager.AddParameter(new Gh_Euc3D_Param.Param_HeMesh(), "Dual Trimmed Mesh", "D", "Dual mesh trimmed with the specified boundary polylines.", GH_Kernel.GH_ParamAccess.item);
        }
        ...
        protected override void SolveInstance(GH_Kernel.IGH_DataAccess DA)
        {
            ...
            Euc3D.Vector normal = Euc3D.Vector.WorldZ;
            ...
            #region Prepare Connected Domain
            ...
            #region Translate to Halfedge Mesh
            ...
            #region Create Dual Halfedge Mesh

            He.Mesh<Euc3D.Point> heDual = new He.Mesh<Euc3D.Point>();

            IReadOnlyList<He.Face<Euc3D.Point>> faces = hePrimal.GetFaces();
            for (int i = 0; i < faces.Count; i++)
            {
                He.Face<Euc3D.Point> face = faces[i];

                Euc3D.Point centroid = new Euc3D.Point(0d, 0d, 0d);
                IReadOnlyList<He.Vertex<Euc3D.Point>> faceVertices = face.FaceVertices();
                for (int j = 0; j < faceVertices.Count; j++) { centroid += faceVertices[j].Position; }
                centroid /= faceVertices.Count;

                heDual.AddVertex(centroid);
            }

            IReadOnlyList<He.Vertex<Euc3D.Point>> vertices = hePrimal.GetVertices();
            for (int i = 0; i < vertices.Count; i++)
            {
                He.Vertex<Euc3D.Point> vertex = vertices[i];
                if (vertex.IsBoundary()) { continue; }

                List<int> i_AdjacentVertices = new List<int>();
                IReadOnlyList<He.Face<Euc3D.Point>> adjacentVertices = vertex.AdjacentFaces();
                for (int j = 0; j < adjacentVertices.Count; j++) { i_AdjacentVertices.Add(adjacentVertices[j].Index); }

                heDual.AddFace(i_AdjacentVertices);
            }
            ...
            // ----- Translate to Halfedge Mesh ----- //

            DA.SetData(0, hePrimal);
            DA.SetData(1, heDual);
        }
        ...
        public override Guid ComponentGuid => new Guid("{BAC8D568-776F-47C1-8A69-9683B7E2E454}");
